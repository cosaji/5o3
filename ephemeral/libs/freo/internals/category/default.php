...
function freo_main()
{
	...
	//パラメータ検証
	if (!isset($_GET['category']) and isset($freo->parameters[1])) {
		$parameters = array();
		$i          = 0;
	...
	if (!isset($_GET['category']) or !preg_match('/^[\w\-\/]+$/', $_GET['category'])) {
		freo_error('表示したいカテゴリーを指定してください。');
	}
