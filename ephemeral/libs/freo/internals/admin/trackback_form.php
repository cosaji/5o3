...
/*********************************************************************
 ...
 freo | 管理画面 | トラックバック入力 (2010/09/01)
...
function freo_main()
{
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (!$freo->smarty->get_template_vars('errors')) {
			$errors = freo_validate_trackback('update', $_POST);
		...
		if ($freo->smarty->get_template_vars('errors')) {
			...
			if (isset($_POST['preview'])) {
				//プレビューへ移動
				freo_redirect('admin/trackback_preview?id=' . $_GET['id']);
			} else {
				//登録処理へ移動
				freo_redirect('admin/trackback_post?freo%5Btoken%5D=' . freo_token('create') . '&id=' . $_GET['id']);
			}
		}
	} else {
		if (!empty($_GET['session']) and !empty($_SESSION['input'])) {
			...
			//編集データ取得
			$stmt = $freo->pdo->prepare('SELECT * FROM ' . FREO_DATABASE_PREFIX . 'trackbacks WHERE id = :id');
			...
			if ($data = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				freo_error('指定されたトラックバックが見つかりません。');
			}
		}
	}
