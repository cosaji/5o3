...
function freo_main()
{
	...
	//オプションファイル削除
	freo_rmdir(FREO_FILE_DIR . 'page_options/' . $_GET['id'] . '/');

	//関連データ削除
	freo_associate_page('delete', $_GET['id']);

	//イメージ削除
	freo_rmdir(FREO_FILE_DIR . 'page_images/' . $_GET['id'] . '/');

	//サムネイル削除
	freo_rmdir(FREO_FILE_DIR . 'page_thumbnails/' . $_GET['id'] . '/');

	//ファイル削除
	freo_rmdir(FREO_FILE_DIR . 'page_files/' . $_GET['id'] . '/');
