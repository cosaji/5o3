...
function freo_execute($function = null)
{
	...
	//キャッシュ判別
	if (FREO_CACHE_MODE and !$freo->user['id'] and $_SERVER['REQUEST_METHOD'] == 'GET' and empty($_GET)) {
		...
		if (!$freo->smarty->template_exists($template)) {
			...
			while (!empty($parameters)) {
				...
				if (!empty($parameters)) {
					...
					if ($freo->smarty->template_exists($template . $path . '/default.html')) {
...
function freo_token($type, $name = 'default')
{
	...
	if ($type == 'check') {
		if ($_REQUEST['freo']['token'] and $_REQUEST['freo']['token'] == $_SESSION['freo']['token'][$name]['value']) {
			$flag = true;
		} else {
			$flag = false;
		}

		if (time() - $_SESSION['freo']['token'][$name]['time'] > FREO_TOKEN_SPAN) {
