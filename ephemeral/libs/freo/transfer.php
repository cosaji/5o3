...
function freo_transfer_link($data)
{
	...
	if (preg_match('/^(' . preg_quote(FREO_HTTP_URL, '/') . '|' . preg_quote(FREO_HTTPS_URL, '/') . ')/', $data)) {
		if (strpos($data, '?')) {
			$data .= '&amp;';
		} else {
			$data .= '?';
...
function freo_transfer_form($data)
{
	...
	if (preg_match('/action="(' . preg_quote(FREO_HTTP_URL, '/') . '|' . preg_quote(FREO_HTTPS_URL, '/') . ')[^\"]*"/', $data)) {
