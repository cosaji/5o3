...
/* メイン処理 */
function freo_display_page_menu()
{
	global $freo;

	//ページ取得
	$stmt = $freo->pdo->prepare('SELECT * FROM ' . FREO_DATABASE_PREFIX . 'pages WHERE pid IS NULL AND approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND display = \'publish\' AND (close IS NULL OR close >= :now2) ORDER BY sort, id');
	...
	$freo->smarty->assign(array(
		'plugin_page_menus'           => $pages,
		'plugin_page_menu_associates' => $page_associates,
		'plugin_page_menu_filters'    => $page_filters,
		'plugin_page_menu_securities' => $page_securities,
		'plugin_page_menu_tags'       => $page_tags,
		'plugin_page_menu_files'      => $page_files,
		'plugin_page_menu_thumbnails' => $page_thumbnails,
		'plugin_page_menu_images'     => $page_images
	));
