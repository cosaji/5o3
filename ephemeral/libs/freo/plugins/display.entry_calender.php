...
function freo_display_entry_calender()
{
	...
	if (FREO_DATABASE_TYPE == 'mysql') {
		$stmt = $freo->pdo->prepare('SELECT datetime FROM ' . FREO_DATABASE_PREFIX . 'entries WHERE approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND display = \'publish\' AND DATE_FORMAT(datetime, \'%Y%m\') = :month AND (close IS NULL OR close >= :now2)');
	} else {
		$stmt = $freo->pdo->prepare('SELECT datetime FROM ' . FREO_DATABASE_PREFIX . 'entries WHERE approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND display = \'publish\' AND STRFTIME(\'%Y%m\', datetime) = :month AND (close IS NULL OR close >= :now2)');
	}
